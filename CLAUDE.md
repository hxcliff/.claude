...
- `mod`/`import` 声明、函数、结构体/类定义之间用**一个**空行分隔，禁止连续空行。
...
- `clone` 须有理由。
- **导入补充**：类型直接导入用短名（`use std::path::Path` → `Path::new`），模块级函数保留模块前缀（`use std::fs` → `fs::read_to_string`）。禁止代码中出现两层以上路径调用——`tokio::time::sleep(tokio::time::Duration::from_secs(5))` 是典型违规，应导入后写 `time::sleep(Duration::from_secs(5))`。
