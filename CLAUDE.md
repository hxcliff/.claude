...
- **出错先想为什么，不许瞎试**：碰到报错，先分析根因说清楚原因，再动手改。不要 A 不行试 B、B 不行又回 A 来回打转。
- **一个方案失败，先停下来说清楚**：说明为什么失败、失败在哪，然后再决定是调整还是换路。不许闷头换方向。
- **改动开始膨胀就停**：发现在不断打补丁、越改越长，立刻停下来重新审视结构，不要水多加面、面多加水。
- **修错误是为了推进目标，不是为了消灭报错**：每次修 bug 前先回头看当前目标是什么，判断这条路还对不对。路不对就换路，不要无脑堵报错。
...
- **导入规则**：类型直接导入用短名（`use std::path::Path` → `Path::new`），模块级函数保留模块前缀（`use std::fs` → `fs::read_to_string`）。代码中禁止写全路径调用。
...
- **导入规则**：通过包的公开导出入口导入，不直接引用 `package:xxx/src/...` 内部路径。命名冲突时才用 `as` 前缀，不要无理由加。
